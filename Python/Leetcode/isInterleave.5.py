...
class Solution():
    ...
    """def setReversed(self,rcp,n):
        ...
        for (i,v) in enumerate(rcp):
            Reversed=True if i>=(len(rcp)-n) else False
    ...
    def RetractRCP(self,rcp):
        ...
        for (j,v) in enumerate(rcp):
            Reversed=True if j>=(len(rcp)-n) else False
            newrcp.append((v[0],v[1],v[2],v[3],v[4],v[5],Reversed))
    ...
    def StringMapper(self, rcp, s1, s2, s3):
        ...
        if s1!="":
            if s1[0]==s3[0] and not self.AlreadyExistsInRCP(self.rcp, ("s1",s1[0],s1,s2,s3,True,True)):
                news1=s1[1:]
                news2=s2
                news3=s3[1:]
                chosen="s1"
        ...
        if s2!="":
            if s2[0]==s3[0] and chosen=="" and not self.AlreadyExistsInRCP(self.rcp, ("s2",s2[0],s1,s2,s3,True,True)):
                news1=s1
                news2=s2[1:]
                news3=s3[1:]
                chosen="s2"
